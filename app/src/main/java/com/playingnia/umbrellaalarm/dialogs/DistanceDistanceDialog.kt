...
class DistanceDistanceDialog(private val context: Context): Dialog(context, android.R.style.Theme_Material_Dialog_NoActionBar) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.editTextDistance.setText(SettingManager.getDistanceDistance().toString())

        binding.textSave.setOnClickListener {
            val distance = binding.editTextDistance.text.toString()
            val set = SettingManager.setDistanceDistance(distance)
            if (set) {
